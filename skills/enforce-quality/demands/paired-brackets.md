...
Treat every ecosystem as a custom-checker case for
  this demand until a native rule that matches the
  full specification appears.

## Custom checker fallback

Ship a lexer-based checker that scans every
  production source file token by token, pairs every
  opening bracket with its matching closing bracket,
  and fails the build on any pair whose closer is
  neither on the opener's line nor on a line whose
  first non-whitespace column matches the column of
  the opener-line's first non-whitespace character.

The checker must handle strings, character
  literals, comments, and language-specific
